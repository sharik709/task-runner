...
class Schedule(BaseModel):
    ...
    interval: Optional[str] = Field(None, description="Interval for recurring tasks (e.g., '1m', '1h', '1d', '1y')")
    ...
    def validate_interval(cls, v, info):
        ...
        if v and not v.endswith(("m", "h", "d", "y")):
            raise ValueError("Interval must end with m (minutes), h (hours), d (days), or y (years)")
        return v
...
class Task(BaseModel):
    ...
    schedule: Schedule
    retry: RetryConfig
    dependencies: List[str] = Field(default_factory=list, description="Names of tasks that must complete before this task")
